...
focus on performance. An array processor is a grid of processing elements, where each of the
processing elements are only able to communicate with its north, south, east and
west neighbours. All the processing elements will perform the same instruction
at all times. After some calculations\TODO{Should we show the actual
  calculations? YES.}, we decided to escalate our goal to a parallel image processor
